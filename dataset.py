...
def getData(corpusFile,sequence_length,batchSize):
    ...
    stock_data = read_csv(corpusFile,dtype={'close': np.float})
    stock_data=stock_data[['close','open','high','low','vol']]
    # stock_data=stock_data[['close','open','high','low','pchangre_close','e','pct_chg','vol','amount']]
    ...
    # stock_data.drop('exchange', axis=1, inplace=True)
    # stock_data.drop('symbol', axis=1, inplace=True)
    # stock_data.drop('freq', axis=1, inplace=True)
    # stock_data.drop('trade_time', axis=1, inplace=True)
    # stock_data.drop('is_contract', axis=1, inplace=True)
    ...
    trainx, trainy = X[:int(0.98 * total_len)], Y[:int(0.98 * total_len)]
    testx, testy = X[int(0.98 * total_len):], Y[int(0.98 * total_len):]
