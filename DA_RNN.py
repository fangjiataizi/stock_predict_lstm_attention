...
from xlsxwriter import utility as util
...
class da_rnn:
    def __init__(self, file_data, logger, encoder_hidden_size = 64, decoder_hidden_size = 64, T = 10,
        ...
        self.X =dat[['open', 'high', 'low', 'vol']].values
        self.y = np.array(dat.close)
    ...
    def train(self, n_epochs = 10):
        ...
        for i in range(n_epochs):
            ...
            while j < self.train_size:
                ...
                # print('loss index:',i * iter_per_epoch + j / self.batch_size)
    ...
    def train_iteration(self, X, y_history, y_target):
        ...
        input_weighted, input_encoded = self.encoder(Variable(torch.from_numpy(X).type(torch.FloatTensor).cpu()))
        y_pred = self.decoder(input_encoded, Variable(torch.from_numpy(y_history).type(torch.FloatTensor).cpu()))
    ...
    def predict(self, on_train = False):
        ...
        while i < len(y_pred):
            ...
            _, input_encoded = self.encoder(Variable(torch.from_numpy(X).type(torch.FloatTensor).cpu()))
